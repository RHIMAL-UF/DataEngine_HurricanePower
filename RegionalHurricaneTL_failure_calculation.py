...
class RegionalHurricaneTLFailureCal:
    ...
    def line_failure_probability_calculation(self, TLFragility, v10_angle, v10_speed, time_interval):
        ...
        path = os.path.join(path, folder_name)
        os.makedirs(path, exist_ok=True)
        with open(f"{path}/LineFailureProbability_at_{string_ts}hour.json", 'w') as json_file:
    ...
    def transmission_line_failure_visu(self, Linefailure, time_interval, boundary):
        ...
        start_time = time.time()
        boudnary = loadmat(f'{boundary}.mat')['boundary']
        ...
        fimage = plt.imread('map04.jpg')
        img = Image.open('map04.jpg').convert("L")
        arr = np.asarray(img)
        ...
        for i in tqdm(range(tts), desc='Transmission_Line_Failure_Visu Loading...', ascii=False,
            ...
            path = os.getcwd()
            folder_name = f'{self.opfolder}/{string_ts}hour'
            path = os.path.join(path, folder_name)
            os.makedirs(path, exist_ok=True)
            plt.savefig(f'{path}/{timestamp[i]}.png', bbox_inches='tight', dpi=1200)
        print(f'Transmission_Line_Failure_Visu Time: {time.time() - start_time}sec')

    def creating_video(self, time_interval):
        # create video
        start_time = time.time()
        if abs(time_interval - 0.5) < 1.0e-4:
            string_ts = 'Half'
        ...
        image_folder = f'{os.getcwd()}/{self.opfolder}/{string_ts}hour'
        video_name = f'{os.getcwd()}/{self.opfolder}/{string_ts}hour/{string_ts}.avi'
    ...
    def playing_video(self, time_interval):
        ...
        if self.pvideo:
            if sys.platform == 'win32':
                os.startfile(f'{os.getcwd()}/{self.opfolder}/{string_ts}hour/{string_ts}.avi')
            else:
                opener = "open" if sys.platform == "darwin" else "xdg-open"
                subprocess.call([opener, f'{os.getcwd()}/{self.opfolder}/{string_ts}hour/{string_ts}.avi'])
